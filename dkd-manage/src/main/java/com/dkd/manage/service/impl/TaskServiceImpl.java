...
public class TaskServiceImpl implements ITaskService 
{
    ...
    public int insertTaskDto(TaskDto taskDto) {
        // 1.查询售货机是否存在
        VendingMachine vendingMachine = vendingMachineService.selectVendingMachingeByInnerCode(taskDto.getInnerCode());
    ...
    public int cancelTask(Task task) {
        ...
        // 2.设置更新字段
        task.setTaskStatus(DkdContants.TASK_STATUS_CANCEL); // 工单状态为取消
    ...
    private void checkCreateTask(Long vmStatus, Long productTypeId) {
        ...
        if(productTypeId == DkdContants.TASK_TYPE_DEPLOY && vmStatus == DkdContants.VM_STATUS_RUNNING) {
            throw new ServiceException("设备在运行中，无法进行投放工单");
        }
