...
public class EmpController extends BaseController
{
    ...
    public AjaxResult buinessList(@PathVariable("innerCode") String innerCode) {
        // 1.根据innerCode查询售货机信息
        VendingMachine vendingMachine = vendingMachineService.selectVendingMachingeByInnerCode(innerCode);
    ...
    public AjaxResult operationList(@PathVariable("innerCode") String innerCode) {
        ...
        Emp emp = new Emp();
        emp.setRegionId(vendingMachine.getRegionId()); // 设备所属区域id
        emp.setRoleCode(DkdContants.ROLE_CODE_OPERATOR); // 角色编码，运营人员
        emp.setStatus(DkdContants.EMP_STATUS_NORMAL); // 员工启用
        return success(empService.selectEmpList(emp));
    }
}
