...
function handleDelete(row) {
  ...
  proxy.$modal.delete('你确定要删除本条内容吗？').then(async () => {
    ...
    let res;
    if(row.classId) {
      res = await listSku({ ...loadAllParams, classId: row.classId })
      if(res.total > 0) flag = true
    }
    else {
      for (let i = 0; i < _classIds.length; i++) {
        res = await listSku({ ...loadAllParams, classId: _classIds[i] })
        if(res.total > 0) {
          flag = true
          break
        }
    ...
    if(flag) return proxy.$modal.msg("请先修改类目下商品后再删除");
    await delSkuClass(_classIds);
    getList();
