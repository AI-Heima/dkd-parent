  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="policyRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="策略名称" prop="policyName">
          <el-input maxlength="15" v-model="form.policyName" placeholder="请输入策略名称" />
        </el-form-item>
        <el-form-item label="策略方案" prop="discount">
          <el-input @input="e => form.discount = e.replace(/[^\d]/g, '')" v-model="form.discount" placeholder="请输入策略方案" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 查看策略详情对话框 -->
    <el-dialog title="策略详情" v-model="readOpen" width="500px" append-to-body>
      <el-form-item label="策略名称" prop="policyName">
        <el-input v-model="form.policyName" readonly />
      </el-form-item>
      <label>包含设备：</label>
      <el-table :data="vmList" style="margin-top: 10px;">
        <el-table-column label="序号" type="index" width="50" align="center" />
...
const data = reactive({
  ...
  rules: {
    ...
    discount: [
      ...
      // 策略方案只能为1-100以内的整数
      { pattern: /^([1-9]\d?|(100))$/, message: "策略方案只能为1-100以内的整数", trigger: "blur" }
    ],
  }
});
...
function handleRead(row) {
  ...
  form.value = {...row};
  // 查看设备列表
  listVm({ ...loadAllParams, policyId: row.policyId }).then(response => {
...
function handleDelete(row) {
  ...
  proxy.$modal.delete('你确定要删除本条内容吗？').then(async () => {
    ...
    let res;
    if(row.policyId) {
      res = await listVm({ ...loadAllParams, policyId: row.policyId })
      if(res.total > 0) flag = true
    }
    else {
      for (let i = 0; i < _ids.length; i++) {
        res = await listVm({ ...loadAllParams, policyId: _ids[i] })
        if(res.total > 0) {
          flag = true
          break
        }
    ...
    if(flag) return proxy.$modal.msg("策略正在应用不能进行删除操作");
...
function querySearch(queryString, cb) {
  const results = queryString !== 'null'
    ? policyList.value.filter(item => item.policyName.indexOf(queryString) === 0)
    : policyList.value
  cb(results)
}
