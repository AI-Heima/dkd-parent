  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body :before-close="cancel">
      <el-form ref="skuRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="商品名称" prop="skuName">
          <el-input maxlength="20" v-model="form.skuName" placeholder="请输入商品名称" />
        </el-form-item>
        <el-form-item label="品牌" prop="brandName">
          <el-input maxlength="20" v-model="form.brandName" placeholder="请输入品牌" />
        </el-form-item>
        <el-form-item label="商品价格" prop="price">
          <el-input-number style="width: 50%;" :precision="2" :step="0.5" :min="0.01" :max="999.99" v-model="form.price" placeholder="请输入商品价格" />&nbsp;元
        </el-form-item>
        <el-form-item label="商品类型" prop="classId">
          <el-select style="width: 100%;" v-model="form.classId" placeholder="请输入商品类型" clearable filterable>
            <el-option
        ...
        <el-form-item label="规格" prop="unit">
          <el-input maxlength="20" v-model="form.unit" placeholder="请输入规格" />
        </el-form-item>
        <el-form-item label="商品图片" prop="skuImage">
          <image-upload v-model="form.skuImage" :limit="1" :fileType="['png', 'jpg']" :fileSize="2" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 数据导入对话框 -->
    <el-dialog title="数据导入" v-model="excelOpen" width="330px" append-to-body :before-close="closeExcelOpen">
      <el-upload
        ref="uploadRef"
        class="upload-demo"
        :limit="1"
      ...
      >
        <template #trigger>
          <el-button style="margin-left: 50px;" type="primary">上传文件</el-button>
        </template>

        <el-button style="margin-left: 40px;" class="ml-3" type="success" @click="submitUpload">
          上传
        </el-button>
...
const data = reactive({
  ...
  rules: {
    ...
    unit: [
      ...
      // 规格只能为数字、字母
      { pattern: /^[a-zA-Z0-9]+$/, message: "规格只能为数字、字母", trigger: "blur" },
    ],
    price: [
...
function handleBeforeUpload(file) {
  ...
  if (file.size / 1024 / 1024 > 1) {
    proxy.$modal.msgError('上传excel文件大小不能超过 1 MB!');
    return false;
  }
