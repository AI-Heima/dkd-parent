    ...
    >
      ...
      <el-form-item label="工单状态" prop="taskStatus">
        <el-radio-group v-model="queryParams.taskStatus">
          <el-radio @click.native.stop.prevent="getCurrentRow(dict.value)" v-for="dict in task_status" :key="dict.value" :label="dict.value" :value="dict.value">{{ dict.label }}</el-radio>
        </el-radio-group>
      </el-form-item>
      <el-form-item>
...
const taskInfo = () => {
  ...
  getTask(taskId.value).then((response) => {
    ...
    getAssignorByAssignorId(form.value.assignorId).then(res => {
      form.value.assignorName = res.data.user_name;
    })
  });
  // 获取货道列表
...
// 再次点击取消单选选中状态
function getCurrentRow(value) {
  if(value == queryParams.value.taskStatus) queryParams.value.taskStatus = null
  else queryParams.value.taskStatus = value
};

/** 输入框显示联想词 */
function querySearch(queryString, cb) {
  const results = queryString !== 'null'
    ? taskList.value.filter(item => item.taskCode.indexOf(queryString) === 0)
    : taskList.value
  cb(results)
}
